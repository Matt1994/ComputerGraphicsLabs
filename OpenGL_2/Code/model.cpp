 ...
 */
void Model::unitize() {
    float maxX = getVertices().at(0).x();
    float minX = getVertices().at(0).x();
    float maxY = getVertices().at(0).y();
    float minY = getVertices().at(0).y();
    float maxZ = getVertices().at(0).z();
    float minZ = getVertices().at(0).z();

    for(QVector3D vertex : getVertices()){ // get min and max
    ...
    for(QVector3D vertex : getVertices()){ // unitize
        int idx = getVertices().indexOf(vertex);

        vertex.setX((vertex.x() - centerX) / maxim);
        vertex.setY((vertex.y() - centerY) / maxim);
        vertex.setZ((vertex.z() - centerZ) / maxim);

        vertices.replace(idx, vertex);
    }
